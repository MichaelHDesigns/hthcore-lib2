...
export class Transaction {
   ...
   *
   * @param {Object} opts allows to skip certain tests. {@see Transaction#serialize}
   * @return {hthcore.Error}
   */
  getSerializationError(opts: any): hthcore.Error;
   ...
   * and a slightly fancier format recognized by hthcore:
   *
   * ```
   * {
   ...
   * }
   * ```
   * Where `address` can be either a string or a hthcore Address object. The
   * same is true for `script`, which can be a string or a hthcore Script.
   *
