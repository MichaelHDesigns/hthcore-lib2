  ...
  [
    function (next) {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        br.writeUInt64LEBN(new hthcore.crypto.BN(num));
        buffers.push(br.toBuffer());

        // hashes
        var data = hthcore.crypto.Hash.sha256sha256(Buffer.alloc(32));
    ...
    function (next) {
      ...
      var suite = new benchmark.Suite();
      suite.add('hthcore', hthcoreTest, { maxTime: maxTime });
